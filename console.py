...
class HBNBCommand(cmd.Cmd):
    ...
    def preloop(self):
        """
            handles the intro to command interpreter
        """
        print('.----------------------------.')
    ...
    def __class_err(self, arg):
        """
            private: checks for missing class or an unknown class
        """
        error = 0
        if len(arg) == 0:
    ...
    def __parse_string(self, value):
        """ parses attribute value passed as a string."""
    ...
    def __parse_number(self, value):
        """ parses attribute value passed as a number."""
        if value.find('.') != -1:
            try:
                value = float(value)
            except:
    ...
    def do_create(self, arg):
        """create: create [ARG]
        ARG = Class Name
        SYNOPSIS: Creates a new instance of Class from the given input ARG"""
        arg = arg.split()
        error = self.__class_err(arg)
        if not error:
    ...
    def do_show(self, arg):
        ...
        ARG = Class
        ARG1 = ID #
        SYNOPSIS: Prints an object of given ID from given Class"""
        arg = arg.split()
        error = self.__class_err(arg)
        if not error:
    ...
    def __check_dict(self, arg):
        """This checks if the arguments input has a dictionary"""
        if '{' and '}' in arg:
            l = arg.split('{')[1]
            l = l.split(', ')
    ...
    def __handle_update_err(self, arg):
        """This checks for all errors in update."""
    ...
    def do_BaseModel(self, arg):
        """The Class method with .function() syntax
